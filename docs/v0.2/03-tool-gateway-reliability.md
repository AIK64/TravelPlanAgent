...
| 可重试 timeout/connection/upstream | 指数退避加 jitter，至多 `TOOL_MAX_ATTEMPTS` | 成功则继续 |
...
缓存容量由 `TOOL_CACHE_MAX_ENTRIES` 约束；POI 与路线 TTL 分别由 `POI_CACHE_TTL_SECONDS`、`ROUTE_CACHE_TTL_SECONDS` 控制；`TOOL_MAX_CONCURRENCY` 的 semaphore 限制同时在飞的实际请求。失败结果不缓存，避免短暂故障被固定。
